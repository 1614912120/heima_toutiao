...
public class ReptilesArticleData {
    ...
    public void initDriver(){
        ...
        driver.get("https://m.163.com/touch/news/sub/international?ver=c&clickfrom=index2018_header_main");
...
//                .get();
        // 找到指定class的所有元素
        Elements elementsByClass = document.getElementsByClass("widget-flowList");
        System.out.println("div元素个数:  " + elementsByClass.size());
        Element element = elementsByClass.get(0);
        // 找到所有article标签页面元素
        Elements li_list = element.getElementsByTag("li");
        ...
        for (Element li : li_list) {
            ...
            try {
                ...
                imgList = li.getElementsByTag("img");
                // 获取封面图片元素集合
            } catch (Exception e) {
                e.printStackTrace();
                continue;
...
//                    WmUser wmUser1 = wmUserMapper.selectOne(Wrappers.<WmUser>lambdaQuery().eq(WmUser::getApUserId, user.getId()));
//                    Integer id = wmUser1.getId();
...
//            String url = "//nimg.ws.126.net/?url=http%3A%2F%2Fdingyue.ws.126.net%2F2020%2F0227%2Fd5ba8ca7j00q6cggh000vc000hs00b7m.jpg&thumbnail=234x146&quality=85&type=jpg";
    ...
    private List<Map> parseContent(String href)  {
        String url="https://m.163.com"+href;
        ...
        try {
            document = Jsoup.connect(url)
                    .header("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.183 Safari/537.36")
                    .get();
                Elements contentDiv = document.getElementsByClass("article-body js-article-body");
//                Element pageDiv = contentDiv.getElementsByTag("p").get(0);
//                Element pageDiv = contentDiv;
//                Elements allElements = pageDiv.getAllElements();
            for (Element contentDivs : contentDiv) {
                Elements p = contentDivs.getElementsByTag("p");
                for (Element elementp : p) {
                    Map map = new HashMap();
                    map.put("type","text");
                    map.put("value",elementp.text());
                    contentMap.add(map);
                    System.out.println("文本内容: " + elementp.text());
                }
            }

//            for (Element subElement : contentDiv) {
